...
path_log_file = open("path_log.jsonl", "w", encoding="utf-8")
...
try:
    while (time.time() - start_time) < TOTAL_DURATION_SECONDS:
        ...
        if current_app['package'] != TARGET_PACKAGE:
            log_action(path_log_file, step_counter, "BACK_FROM_EXTERNAL", current_activity)
            d.press("back")
            time.sleep(1)
            if d.app_current()['package'] != TARGET_PACKAGE:
                d.app_start(TARGET_PACKAGE)
            continue
        ...
        if all_elements.count > 0:
            for el in all_elements:
                ...
                if "退出" not in el_text and "登录" not in el_text and "注销" not in el_text:
                    safe_elements.append(el)

        if not safe_elements:
            log_action(path_log_file, step_counter, "BACK_NO_ELEMENTS", current_activity)
            d.press("back")
            time.sleep(1)
            continue
        ...
        if action_roll < 0.80 and safe_elements:
            element_to_click = random.choice(safe_elements)
            ...
            log_action(path_log_file, step_counter, "BACK_KEY", current_activity)
            d.press("back")
